...
heading = subject[:1].upper() + subject[1:]
...
html_body = f"""<!doctype html><html lang="id"><body style="margin:0;padding:24px 12px;background:#F5F7FA;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;color:#111827"><main style="box-sizing:border-box;width:100%;max-width:600px;margin:0 auto;padding:32px;background:#fff;border:1px solid #E5E7EB;border-radius:12px"><table role="presentation" style="margin:0 0 28px"><tr><td style="padding-right:10px"><img src="https://laprakin.app/brand/laprakin-email-logo.png?v=1" width="36" height="36" alt="Logo Laprakin" style="display:block"></td><td style="font-family:'Plus Jakarta Sans',Arial,sans-serif;font-size:18px;font-weight:700">Laprakin</td></tr></table><div style="font-size:11px;font-weight:700;color:#6B7280;margin-bottom:12px"><span style="display:inline-block;width:8px;height:8px;margin-right:8px;border-radius:999px;background:{color}"></span>{status}</div><h1 style="font-family:'Plus Jakarta Sans',Arial,sans-serif;font-size:23px;line-height:30px;margin:0 0 18px">{html.escape(heading)}</h1><p style="font-size:14px;line-height:21px;margin:0 0 18px">{escaped_body}</p><p style="font-size:13px;line-height:20px;color:#6B7280;margin:0">Host: <span style="font-family:Consolas,monospace;color:#111827">{host}</span></p><hr style="border:0;border-top:1px solid #E5E7EB;margin:28px 0 20px"><p style="font-size:12px;line-height:18px;color:#6B7280;margin:0">Email otomatis dari Laprakin. Tidak perlu membalas email ini.</p></main></body></html>"""
print(json.dumps({
    "from": sender,
