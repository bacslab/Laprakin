...
| Proper i18n migration | Incomplete | `id.json`/`en.json` and keyed translator now cover Auth, admin MFA, the public Landing/Pricing pages, Billing, and dynamic Status labels; the legacy DOM walker and dictionary are isolated behind `I18nRuntime`, but workspace/admin component migration and pluralization remain. |
