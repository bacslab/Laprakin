...
| Accessibility acceptance gate | Incomplete | `eslint-plugin-jsx-a11y` is not active because the current ESLint 10 dependency range conflicts with the available plugin peer range; contrast automation, full image-alt audit, and mobile/tablet visual checks remain. |
