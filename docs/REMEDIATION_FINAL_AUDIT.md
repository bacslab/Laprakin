...
| Proper i18n migration | Incomplete | `id.json`/`en.json` and keyed translator now cover Auth, admin MFA, the public Landing/Pricing pages, Billing, Workspace tutorial, Composer, attachment previews, Help/Feedback/Notifications overlays, document preview/workflow surfaces, all Settings panels including data/storage/safety/security/archived/academic/keyboard, Workspace shell/configuration/notices, sidebar chat controls, document library, Projects, academic identity intake, the Admin Console operational shell plus pricing/access/appeal panels, and dynamic Status labels; the legacy DOM walker and dictionary are isolated behind `I18nRuntime`, but the remaining workspace/admin component migration and pluralization remain. |
...
- Client contract suite: 44 passed, 0 failed.
