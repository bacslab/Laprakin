...
const REASON_OPTIONS = [
  ...
  { value: 'security_incident', labelKey: 'securityIncident' },
  { value: 'legal_request', labelKey: 'legalRequest' },
  { value: 'data_subject_request', labelKey: 'dataSubjectRequest' },
  { value: 'other', labelKey: 'otherReason' },
];

export default function AdminAccessPanel({ users, capabilities = [], selectedUserId = '', onSelectUser, setNotice, onRefresh }) {
  const { t } = useI18n();
  ...
  const [breakGlassForm, setBreakGlassForm] = useState({ reasonCode: 'support_case', reasonNote: '', durationMinutes: '5' });
  ...
  const canRestrict = capabilities.includes('users.restrict');

  useEffect(() => { setSelectedId(selectedUserId || users[0]?.id || ''); }, [selectedUserId, users]);
  useEffect(() => { setPii(null); setContent(null); }, [selectedId]);
  useEffect(() => {
    if (!pii && !content) return undefined;
    const timer = window.setInterval(() => setClock(Date.now()), 1_000);
    return () => window.clearInterval(timer);
  }, [pii, content]);
  useEffect(() => {
    if (pii && Date.parse(pii.expiresAt) <= clock) setPii(null);
    if (content && Date.parse(content.expiresAt) <= clock) setContent(null);
  }, [clock, pii, content]);
  ...
  const loadDetails = useCallback(async (userId) => {
    ...
    try {
      const [roomData, restrictionData] = await Promise.all([
        ...
        canRestrict ? api(`/admin/users/${userId}/restrictions`) : Promise.resolve({ restrictions: [] }),
      ]);
      setRooms(roomData.rooms || []);
      setRestrictions(restrictionData.restrictions || []);
    } catch (error) { setNotice(error.message); }
  }, [canRestrict, setNotice]);
  useEffect(() => { loadDetails(selectedId); }, [selectedId, loadDetails]);
  useEffect(() => {
    setPlanForm({ planKey: ['monthly', 'pro'].includes(selected?.planKey) ? selected.planKey : 'free', durationDays: '30' });
  ...
  const changePlan = async () => {
    ...
    setBusy(true);
    try {
      await api(`/admin/users/${selectedId}/plan`, { method: 'PUT', body: { planKey: planForm.planKey, durationDays: Number(planForm.durationDays || 30) } });
      await onRefresh();
      setNotice(t('admin.console.access.planUpdated', { user: selected?.userRef || t('admin.console.access.user') }));
  ...
  const createRestriction = async () => {
    ...
    setBusy(true);
    try {
      await api(`/admin/users/${selectedId}/restrictions`, { method: 'POST', body: { targetType: form.targetType, durationDays: form.permanent ? null : Number(form.durationDays), reason: form.reason } });
  ...
  const grantInput = () => ({ reasonCode: breakGlassForm.reasonCode, reasonNote: breakGlassForm.reasonNote.trim(), durationMinutes: Number(breakGlassForm.durationMinutes) });

  const revealPii = async () => {
    if (!selectedId || breakGlassForm.reasonNote.trim().length < 12) return;
    setBusy(true);
    try {
      const { access } = await api(`/admin/users/${selectedId}/pii-access`, { method: 'POST', body: grantInput() });
      const result = await api(`/admin/break-glass/${access.id}/pii`);
      setPii({ ...result.pii, accessId: access.id, expiresAt: result.expiresAt });
      setBreakGlassForm((value) => ({ ...value, reasonNote: '' }));
    } catch (error) { setNotice(error.message); } finally { setBusy(false); }
  };

  const revealContent = async (room) => {
    if (!selectedId || breakGlassForm.reasonNote.trim().length < 12) return;
    setBusy(true);
    try {
      const { access } = await api(`/admin/users/${selectedId}/rooms/${room.id}/content-access`, { method: 'POST', body: grantInput() });
      const result = await api(`/admin/break-glass/${access.id}/content`);
      setContent({ ...result.content, accessId: access.id, expiresAt: result.expiresAt });
      setBreakGlassForm((value) => ({ ...value, reasonNote: '' }));
    } catch (error) { setNotice(error.message); } finally { setBusy(false); }
  };

  const clearSensitive = async (value, setter) => {
    setter(null);
    if (!value?.accessId) return;
    try { await api(`/admin/break-glass/${value.accessId}`, { method: 'DELETE' }); } catch { /* expiry still guarantees bounded access */ }
  };
  ...
  return <section className="admin-content admin-access-layout">
    ...
    <div className="admin-access-detail">
      ...
      {(canRevealPii || canRevealContent) && selected && <section className="admin-panel admin-break-glass-panel"><div className="admin-panel-head"><div><h2>{t('admin.console.access.breakGlass')}</h2><small>{t('admin.console.access.breakGlassDescription')}</small></div></div><div className="admin-break-glass-form"><label>{t('admin.console.access.reasonCode')}<CustomSelect ariaLabel={t('admin.console.access.reasonCode')} value={breakGlassForm.reasonCode} onChange={(reasonCode) => setBreakGlassForm((value) => ({ ...value, reasonCode }))} options={REASON_OPTIONS.map((option) => ({ value: option.value, label: t(`admin.console.access.${option.labelKey}`) }))}/></label><label>{t('admin.console.access.accessDuration')}<input aria-label={t('admin.console.access.accessDuration')} type="number" min="1" max="10" value={breakGlassForm.durationMinutes} onChange={(event) => setBreakGlassForm((value) => ({ ...value, durationMinutes: event.target.value }))}/></label><label className="admin-form-wide">{t('admin.console.access.accessNote')}<textarea aria-label={t('admin.console.access.accessNote')} maxLength="500" value={breakGlassForm.reasonNote} onChange={(event) => setBreakGlassForm((value) => ({ ...value, reasonNote: event.target.value }))} placeholder={t('admin.console.access.accessNotePlaceholder')}/></label>{canRevealPii && <Button variant="secondary" onClick={revealPii} disabled={busy || breakGlassForm.reasonNote.trim().length < 12}><Eye size={14}/>{t('admin.console.access.revealIdentity')}</Button>}</div>{pii && <div className="admin-sensitive-reveal" role="status"><div><b>{pii.fullName || t('admin.console.access.nameUnavailable')}</b><span>{pii.email}</span><small>{t('admin.console.access.expiresIn', { seconds: remaining(pii.expiresAt) })}</small></div><button type="button" onClick={() => clearSensitive(pii, setPii)}><EyeOff size={14}/>{t('admin.console.access.hideNow')}</button></div>}</section>}

      {canRestrict && <section className="admin-panel"><div className="admin-panel-head"><h2>{t('admin.console.access.restrictions')}</h2><small>{t('admin.console.access.pseudonymous')}</small></div><div className="admin-list">{restrictions.length ? restrictions.map((item) => <article key={item.id}><div><b>{item.targetType === 'account' ? t('admin.console.access.account') : item.targetType === 'device' ? t('admin.console.access.device') : t('admin.console.access.network')}</b><small>{item.reason} · {item.permanent ? t('admin.console.access.permanentStatus') : t('admin.console.access.until', { date: formatDate(item.expiresAt) })}</small></div><div className="admin-actions"><span className={`status-${item.status === 'active' ? 'failed' : 'completed'}`}>{item.status}</span>{item.status === 'active' && <button type="button" onClick={() => revokeRestriction(item.id)}>{t('admin.console.access.revoke')}</button>}</div></article>) : <p className="empty-admin">{t('admin.console.access.noRestrictions')}</p>}</div></section>}
      <section className="admin-panel"><div className="admin-panel-head"><h2>{t('admin.console.access.rooms')}</h2><small>{t('admin.console.access.usageMetadata')}</small></div><div className="admin-list">{rooms.length ? rooms.map((room) => <article key={room.id}><div><b>{room.roomRef}</b><small>{t('admin.console.access.updated')} {formatDate(room.updatedAt)}</small></div><div className="admin-actions"><span>{t('admin.console.access.messagesTokens', { messages: room.messageCount, tokens: Number(room.totalTokens || 0).toLocaleString('id-ID') })}</span>{canRevealContent && <button type="button" disabled={busy || breakGlassForm.reasonNote.trim().length < 12} onClick={() => revealContent(room)}>{t('admin.console.access.inspectContent')}</button>}</div></article>) : <p className="empty-admin">{t('admin.console.access.noRooms')}</p>}</div>{content && <div className="admin-sensitive-reveal admin-content-reveal" role="status"><div><b>{content.title}</b><small>{t('admin.console.access.expiresIn', { seconds: remaining(content.expiresAt) })}</small>{content.messages.map((message) => <p key={message.id}><strong>{message.role}</strong>: {message.content}</p>)}</div><button type="button" onClick={() => clearSensitive(content, setContent)}><EyeOff size={14}/>{t('admin.console.access.hideNow')}</button></div>}</section>
    </div>
  </section>;
}
