...
export default function CreditsRoute({ setNotice }) {
  ...
  return <AdminRouteState resource={resource}>{(data) => <><AdminListControls query={query} setQuery={setQuery} pageInfo={data.pageInfo}/><section className="admin-content"><div className="admin-grid"><section className="admin-panel admin-credit-panel"><div className="admin-panel-head"><h2>{t('admin.console.credits.title')}</h2><small>{t('admin.console.credits.description')}</small></div><label htmlFor="admin-credit-audience">{t('admin.console.credits.target')}<CustomSelect id="admin-credit-audience" ariaLabel={t('admin.console.credits.targetAria')} value={form.audience} onChange={(audience) => setForm((value) => ({ ...value, audience }))} options={[{ value: 'user', label: t('admin.console.credits.singleUser') }, { value: 'all', label: t('admin.console.credits.verifiedUsers') }, { value: 'paid', label: t('admin.console.credits.paidUsers') }]}/></label>{form.audience === 'user' && <label htmlFor="admin-credit-user">{t('admin.console.credits.user')}<CustomSelect id="admin-credit-user" ariaLabel={t('admin.console.credits.recipientAria')} value={form.userId} onChange={(userId) => setForm((value) => ({ ...value, userId }))} options={[{ value: '', label: t('admin.console.credits.chooseUser') }, ...data.users.map((item) => ({ value: item.id, label: `${item.email} · ${t('admin.console.credits.creditsCount', { count: item.credits })}` }))]}/></label>}<label>{t('admin.console.credits.amount')}<input aria-label={t('admin.console.credits.amountAria')} type="number" min="1" max="100" value={form.amount} onChange={(event) => setForm((value) => ({ ...value, amount: event.target.value }))}/></label><label>{t('admin.console.credits.reason')}<input aria-label={t('admin.console.credits.reasonAria')} maxLength="160" value={form.reason} placeholder={t('admin.console.credits.defaultReason')} onChange={(event) => setForm((value) => ({ ...value, reason: event.target.value }))}/></label><Button onClick={grant} disabled={busy || (form.audience === 'user' && !form.userId)}><CreditCard size={14}/>{t('admin.console.credits.add')}</Button></section><section className="admin-panel"><div className="admin-panel-head"><h2>{t('admin.console.credits.recentUsers')}</h2><small>{t('admin.console.credits.accountCount', { count: data.users.length })}</small></div><div className="admin-list admin-user-list">{data.users.map((item) => <article key={item.id}><div><b>{item.fullName || item.email}</b><small>{item.email} · {item.plan}</small></div><span>{t('admin.console.credits.creditsCount', { count: item.credits })}</span></article>)}</div></section></div></section></>}</AdminRouteState>;
}
