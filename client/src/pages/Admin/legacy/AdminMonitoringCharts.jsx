...
export default function AdminMonitoringCharts({ data, filters, onFilterChange, t, language }) {
  ...
  return <section className="admin-monitoring-analytics" aria-labelledby="admin-monitoring-analytics-title">
    <div className="admin-monitoring-toolbar">
      <div><span className="admin-monitoring-section-kicker"><Sparkles size={14} /> {t('admin.console.monitoring.summary')}</span><h3 id="admin-monitoring-analytics-title">{t('admin.console.monitoring.analyticsTitle')}</h3><p>{t('admin.console.monitoring.period', { days: usage.days || filters.days })}</p></div>
    ...
    <form className="admin-monitoring-filters" onSubmit={(event) => event.preventDefault()} aria-label={t('admin.console.monitoring.filters')}>
      <div className="admin-monitoring-filters-title"><SlidersHorizontal size={16} /><span>{t('admin.console.monitoring.filterTelemetry')}</span></div>
    ...
    <div className="admin-monitoring-chart-grid">
      <section className="admin-monitoring-chart-panel"><header><div><span>{t('admin.console.monitoring.traffic')}</span><h4>{t('admin.console.monitoring.callsAndErrors')}</h4></div><Activity size={17} /></header><div className="admin-chart" role="img" aria-label={t('admin.console.monitoring.trafficChart')}>{rows.length ? <ResponsiveContainer width="100%" height={286}><AreaChart data={rows} margin={{ top: 12, right: 8, left: -12, bottom: 0 }}><defs><linearGradient id="admin-success-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--admin-chart-positive)" stopOpacity={0.3}/><stop offset="95%" stopColor="var(--admin-chart-positive)" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="4 4" stroke="var(--admin-line)"/><XAxis dataKey="day" tickFormatter={shortDate} stroke="var(--admin-muted)" fontSize={11}/><YAxis allowDecimals={false} stroke="var(--admin-muted)" fontSize={11}/><Tooltip contentStyle={{ border: '1px solid var(--admin-line)', borderRadius: 10, background: 'var(--admin-card)', color: 'var(--admin-text)' }}/><Legend/><Area type="monotone" dataKey="successful" name={t('admin.console.monitoring.successful')} stroke="var(--admin-chart-positive)" fill="url(#admin-success-fill)" strokeWidth={2.5}/><Area type="monotone" dataKey="errors" name={t('admin.console.monitoring.errors')} stroke="var(--admin-chart-negative)" fill="transparent" strokeWidth={2.5}/></AreaChart></ResponsiveContainer> : <div className="admin-chart-empty"><Activity size={22} /><b>{t('admin.console.monitoring.noData')}</b><span>{t('admin.console.monitoring.trafficEmptyHint')}</span></div>}</div></section>
      <section className="admin-monitoring-chart-panel"><header><div><span>{t('admin.console.monitoring.performance')}</span><h4>{t('admin.console.monitoring.latencyAndTokens')}</h4></div><Gauge size={17} /></header><div className="admin-chart" role="img" aria-label={t('admin.console.monitoring.performanceChart')}>{rows.length ? <ResponsiveContainer width="100%" height={286}><LineChart data={rows} margin={{ top: 12, right: 8, left: -12, bottom: 0 }}><CartesianGrid strokeDasharray="4 4" stroke="var(--admin-line)"/><XAxis dataKey="day" tickFormatter={shortDate} stroke="var(--admin-muted)" fontSize={11}/><YAxis yAxisId="latency" stroke="var(--admin-muted)" fontSize={11}/><YAxis yAxisId="tokens" orientation="right" stroke="var(--admin-muted)" fontSize={11}/><Tooltip contentStyle={{ border: '1px solid var(--admin-line)', borderRadius: 10, background: 'var(--admin-card)', color: 'var(--admin-text)' }}/><Legend/><Line yAxisId="latency" type="monotone" dataKey="average_latency_ms" name={t('admin.console.monitoring.latencyMs')} stroke="var(--admin-chart-blue)" strokeWidth={2.5} dot={false}/><Line yAxisId="tokens" type="monotone" dataKey="total_tokens" name={t('admin.console.monitoring.tokens')} stroke="var(--admin-chart-violet)" strokeWidth={2.5} dot={false}/></LineChart></ResponsiveContainer> : <div className="admin-chart-empty"><Gauge size={22} /><b>{t('admin.console.monitoring.noData')}</b><span>{t('admin.console.monitoring.performanceEmptyHint')}</span></div>}</div></section>
    </div>
