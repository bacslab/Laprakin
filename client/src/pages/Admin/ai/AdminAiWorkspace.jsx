...
export default function AdminAiWorkspace() {
  ...
  return <div className={`admin-ai-shell ${theme === 'dark' ? 'theme-dark' : 'theme-light'}`}>
    ...
    <main className="admin-ai-main">
      <div className="admin-ai-topbar"><div><span className="admin-ai-environment">{t('common.productionControls')}</span><b>{t('common.configuration')}</b></div><button type="button" className="admin-ai-theme" onClick={() => setTheme((value) => value === 'dark' ? 'light' : 'dark')} aria-label={t('common.changeTheme')}>{theme === 'dark' ? <Sun size={16} /> : <Moon size={16} />}</button></div>
