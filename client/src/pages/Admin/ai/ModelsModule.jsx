...
import { Eye, FlaskConical, Plus, Power, Search } from 'lucide-react';
...
export default function ModelsModule() {
  ...
  const [filters, setFilters] = useState({ q: url.get('q') || '', providerId: url.get('providerId') || '', state: url.get('state') || '', revisionId: url.get('revisionId') || '', cursor: url.get('cursor') || '' });
  ...
  const discover = async () => {
    ...
    try { const result = await client.discoverModels({ revisionId: resource.data.revisionId, providerId: filters.providerId.trim(), reason: reason.trim() }); setMessage(t('models.discoverySaved')); followRevision(result); }
    catch (error) { setMessage(error.message); }
    finally { setBusy(false); }
  };
