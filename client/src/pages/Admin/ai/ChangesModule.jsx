...
export default function ChangesModule() {
  ...
  const previousPage = () => { const current = Number.parseInt(applied.cursor || '0', 10) || 0; const next = { ...applied, cursor: String(Math.max(0, current - 25)) }; setFilters(next); setApplied(next); const query = new URLSearchParams(Object.entries(next).filter(([, value]) => value).map(([key, value]) => [key, value])); navigate(`${ADMIN_AI_PATHS.changes}${query.size ? `?${query}` : ''}`); };
  ...
  return <>
    ...
    <AdminAiPagination cursor={applied.cursor} nextCursor={resource.data?.nextCursor} onNext={() => { const next = { ...applied, cursor: resource.data.nextCursor }; setFilters(next); setApplied(next); }} onPrevious={previousPage} />
