...
import { AlertOctagon, CheckCircle2, FlaskConical, History, Play, RotateCcw, ShieldCheck } from 'lucide-react';
...
export default function ChangesModule() {
  ...
  const [pageNumber, setPageNumber] = useState(url.get('cursor') ? 2 : 1);
  ...
  const updateLocation = (next) => { const query = new URLSearchParams(Object.entries(next).filter(([, value]) => value).map(([key, value]) => [key, value])); navigate(`${ADMIN_AI_PATHS.changes}${query.size ? `?${query}` : ''}`); };
  const applyFilters = (event) => { event.preventDefault(); const next = { ...filters, cursor: '' }; setCursorHistory([]); setPageNumber(1); setApplied(next); updateLocation(next); };
  const nextPage = () => {
    const nextCursor = resource.data?.nextCursor || '';
    if (!nextCursor) return;
    setCursorHistory((history) => [...history, applied.cursor || '']);
    const next = { ...applied, cursor: nextCursor };
    setPageNumber((value) => value + 1); setFilters(next); setApplied(next); updateLocation(next);
  };
  const previousPage = () => {
    const history = cursorHistory;
    const previousCursor = history.at(-1) || '';
    setCursorHistory(history.slice(0, -1));
    const next = { ...applied, cursor: previousCursor };
    setPageNumber((value) => Math.max(1, value - 1)); setFilters(next); setApplied(next); updateLocation(next);
  };
  ...
  return <>
    ...
    <form className="admin-ai-filterbar is-compact" onSubmit={applyFilters}><label><span>{t('changes.status')}</span><CustomSelect ariaLabel={t('changes.status')} value={filters.state} onChange={(state) => setFilters((value) => ({ ...value, state }))} options={[{ value: '', label: t('common.all') }, { value: 'draft', label: 'Draft' }, { value: 'tested', label: t('changes.tested') }, { value: 'active', label: t('changes.active') }, { value: 'superseded', label: t('changes.superseded') }]} /></label><button type="submit">{t('common.apply')}</button></form>
    ...
    <AdminAiPagination cursor={applied.cursor} previousCursor={cursorHistory.at(-1) || ''} page={pageNumber} nextCursor={resource.data?.nextCursor} onNext={nextPage} onPrevious={previousPage} />
