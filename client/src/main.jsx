...
function AppProvider({ children }) {
  ...
  return <AppContext.Provider value={value}>{children}{notice && <div className={`toast toast-${noticeToneFor(notice)}`} role="status"><span>{notice}</span><IconButton label="Tutup notifikasi" onClick={() => setNotice(null)}><X size={14} /></IconButton></div>}<AppDialog dialog={dialog} onResolve={resolveDialog} /></AppContext.Provider>;
}
