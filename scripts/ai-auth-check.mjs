...
const request = async (options = {}) => {
  ...
  if (String(options.url).endsWith('/models')) return new Response(JSON.stringify({ data: [
...
const [{ generateAiContent, aiThinkingConfigFor, selectAiRoute }, { db }, { createOpenAiCompatibleAdapter }] = await Promise.all([
  import('../server/src/ai.js'),
  import('../server/src/db.js'),
  import('../server/src/ai-providers/openai-compatible.js'),
]);
const adapter = createOpenAiCompatibleAdapter({
  id: 'nararouter',
  baseUrl: process.env.NARAROUTER_BASE_URL,
  getToken: () => process.env.NARAROUTER_API_KEY,
  request,
});
const providerAdapters = { get: (providerId) => {
  assert.equal(providerId, 'nararouter');
  return adapter;
} };
const purpose = `contract-${crypto.randomBytes(6).toString('hex')}`;
const generated = await generateAiContent({ purpose, contents: [{ role: 'user', parts: [{ text: 'Tes provider.' }] }], maxOutputTokens: 80, providerAdapters });
assert.equal(generated.text, 'READY');
assert.equal(generated.provider, 'nararouter');
assert.ok(requests.find((providerRequest) => providerRequest.headers?.authorization === 'Bearer nararouter-contract-key'));
assert.ok(requests.every((providerRequest) => !String(providerRequest.url).includes('googleapis')));
...
const structured = await generateAiContent({ purpose: `${purpose}-structured`, contents: [{ role: 'user', parts: [{ text: 'JSON' }] }], maxOutputTokens: 80, responseMimeType: 'application/json', responseJsonSchema: { type: 'object', properties: { status: { type: 'string' } }, required: ['status'] }, providerAdapters });
